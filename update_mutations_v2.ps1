...
"@
$start = $content.IndexOf("function onPaymentSuccess(response) {")
if ($start -gt -1) {
    $end = $content.IndexOf("function onPaymentFailed(error) {", $start)
    $chunk = $content.Substring($start, $end - $start)
    $content = $content.Replace($chunk, "$onPaymentSuccessNew`n`n")
}
...
"@
$start = $content.IndexOf("function saveListing() {")
if ($start -gt -1) {
    $end = $content.IndexOf("function closeListingModal() {", $start)
    $chunk = $content.Substring($start, $end - $start)
    $content = $content.Replace($chunk, "$saveListingNew`n`n")
}
...
"@
$start = $content.IndexOf("function deleteListing(id) {")
if ($start -gt -1) {
    $end = $content.IndexOf("function toggleListingAvailability(", $start)
    $chunk = $content.Substring($start, $end - $start)
    $content = $content.Replace($chunk, "$deleteListingNew`n`n")
}
...
"@
$start = $content.IndexOf("function toggleListingAvailability(toggleEl, id) {")
if ($start -gt -1) {
    $end = $content.IndexOf("function openPreview(", $start)
    $chunk = $content.Substring($start, $end - $start)
    $content = $content.Replace($chunk, "$toggleAvailabilityNew`n`n")
}
...
"@
$start = $content.IndexOf("function toggleHeart(btn) {")
if ($start -gt -1) {
    $end = $content.IndexOf("/* `r`n   RENDER SAVED VIEW", $start)
    if ($end -eq -1) { $end = $content.IndexOf("/*", $start) }
    $chunk = $content.Substring($start, $end - $start)
    $content = $content.Replace($chunk, "$toggleHeartNew`n`n")
}
...
"@
$start = $content.IndexOf("function removeSaved(propId, btn) {")
if ($start -gt -1) {
    $end = $content.IndexOf("/* `r`n   RENDER MY BOOKINGS VIEW", $start)
    if ($end -eq -1) { $end = $content.IndexOf("/*", $start) }
    $chunk = $content.Substring($start, $end - $start)
    $content = $content.Replace($chunk, "$removeSavedNew`n`n")
}
...
if ($start -gt -1) {
    $end = $content.IndexOf("function showAuthSheet() {", $start)
    if ($end -eq -1) { $end = $content.IndexOf("/* `r`n   AUTH LAUNCH SHEET", $start) }
    $chunk = $content.Substring($start, $end - $start)
